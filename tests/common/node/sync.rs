    ...
    pub async fn recv(&self) -> Result<SyncEvent, SyncError> {
        match timeout(SYNC_RECV_TIMEOUT, self.diag_rx.lock().await.recv()).await {
            ...
            Ok(Ok(event)) => match event {
                ...
                DiagnosticEvent::RemoteHeight { height } => {
                    Ok(SyncEvent::RemoteHeight(height))
                }
                DiagnosticEvent::TrustMetric { feedback } => {
                    Ok(SyncEvent::TrustMetric(feedback))
                }
                DiagnosticEvent::TrustNewInterval { report } => {
                    Ok(SyncEvent::TrustReport(report))
                }
