...
impl<Mapping: 'static + ServiceMapping> MutaBuilder<Mapping> {
    ...
    pub fn build(self, listen_port: u16) -> ProtocolResult<Muta<Mapping>> {
        let mut config: Config = common_config_parser::parse(
            &self.config_path.expect("config path is not set"),
        )
        ...
        let genesis_toml =
            fs::read_to_string(&self.genesis_path.expect("genesis path is not set"))
                .map_err(MainError::Io)?;
        let genesis: Genesis =
            toml::from_str(&genesis_toml).map_err(MainError::GenesisTomlDe)?;
