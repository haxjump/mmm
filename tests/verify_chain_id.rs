...
use core_consensus::message::{
    FixedBlock, FixedHeight, BROADCAST_HEIGHT, RPC_RESP_SYNC_PULL_BLOCK,
    RPC_SYNC_PULL_BLOCK,
};
use core_network::{
    DiagnosticEvent, NetworkConfig, NetworkService, NetworkServiceHandle, PeerId,
    PeerIdExt,
};
use derive_more::Display;
use protocol::traits::{
    Context, MessageCodec, MessageHandler, Priority, Rpc, TrustFeedback,
};
...
fn should_be_disconnected_due_to_different_chain_id() {
    ...
    local.block_on(&mut rt, async move {
        ...
        tokio::task::spawn_local(full_node::run(
            full_port,
            full_seckey.clone(),
            sync.clone(),
        ));
        ...
        network
            .register_endpoint_handler(
                BROADCAST_HEIGHT,
                ReceiveRemoteHeight(sync.clone()),
            )
