...
use protocol::traits::{
    Context, Executor, ExecutorParams, ExecutorResp, Service, ServiceMapping,
...
pub struct ServiceStateMap<DB: TrieDB>(
    HashMap<String, Rc<RefCell<GeneralServiceState<DB>>>>,
);
    ...
    ServiceExecutor<S, DB, Mapping>
{
    pub fn create_genesis(
        ...
        let sdk_factory =
            DefaultSDKFactory::new(Rc::clone(&states), Rc::clone(&querier));
        ...
        for params in services.into_iter() {
            ...
            .map_err(|e| {
                ProtocolError::from(ExecutorError::InitService(format!("{:?}", e)))
            })?;
    ...
    fn get_service(
        &self,
        service: &str,
    ) -> ProtocolResult<Rc<RefCell<Box<dyn Service>>>> {
        self.services
            .get(service)
            .map(|s| Rc::clone(s))
            .ok_or_else(|| {
    ...
    fn get_tx_hooks(&self, exec_type: ExecType) -> Box<dyn TxHooks> {
        match exec_type {
            ...
            ExecType::Write => {
                ...
                for name in self.service_mapping.list_service_name().into_iter() {
                    let tx_hook_service =
                        self.get_service(name.as_str()).expect("no service");
        ...
        let ret = if resp.iter().any(|r| r.is_error()) {
            ...
            match panic::catch_unwind(AssertUnwindSafe(|| {
                ...
                Err(e) => {
                    self.revert_cache()?;
                    log::error!(
                        "inner chain error occurred when calling service: {:?}",
                        e
                    );
...
impl<S: 'static + Storage, DB: 'static + TrieDB, Mapping: 'static + ServiceMapping>
    Executor for ServiceExecutor<S, DB, Mapping>
{
    #[muta_apm::derive::tracing_span(
        kind = "executor.exec",
        logs = "{'tx_len': 'txs.len()'}"
    ...
    fn read(
        ...
        panic::catch_unwind(AssertUnwindSafe(|| self.call(context, ExecType::Read)))
            .map_err(|e| {
                ProtocolError::from(ExecutorError::QueryService(format!("{:?}", e)))
            })
    }
}
