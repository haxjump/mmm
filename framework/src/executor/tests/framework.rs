...
use protocol::types::{
    ...
    ServiceContext, SignedTransaction, TransactionRequest,
};
...
fn test_tx_hook_ok_ok() {
    ...
    let resp: ExecutorResp =
        exec_txs!(50000, 50000, ("TestService", "test_write", "before"));
    ...
    let resp: ExecutorResp =
        exec_txs!(50000, 50000, ("TestService", "test_write", "after"));
    ...
    let resp: ExecutorResp =
        exec_txs!(50000, 50000, ("TestService", "test_write", "before_after"));
    assert_eq!(1, resp.receipts.get(0).unwrap().events.len());
    assert!(
        resp.receipts
            .get(0)
