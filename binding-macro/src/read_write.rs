...
fn verify_inputs(inputs: &Punctuated<FnArg, Token![,]>, mutable: bool) {
    if inputs.len() < 2 || inputs.len() > 3 {
        panic!(
            "The input parameters should be `(&self/&mut self, ctx: ServiceContext)` or `(&self/&mut self, ctx: ServiceContext, payload: PayloadType)`"
        )
    }
...
fn verify_ret_type(ret_type: &ReturnType) {
    let real_ret_type = match ret_type {
        ...
        _ => panic!(
            "The return type of read/write method must be protocol::ProtocolResult"
        ),
    };
