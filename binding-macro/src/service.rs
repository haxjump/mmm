...
pub fn gen_service_code(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let list_method_meta: Vec<MethodMeta> =
        methods.into_iter().map(extract_method_meta).collect();
...
fn get_genesis_body(item: &ImplItemMethod) -> proc_macro2::TokenStream {
    ...
    match item.sig.inputs.len() {
        ...
        _ => panic!(
            "genesis method input params should be `(&mut self)` or `(&mut self, payload: PayloadType)`"
        ),
    }
}
...
fn extract_method_meta(method: ServiceMethod) -> MethodMeta {
    ...
    match &impl_method.sig.inputs.len() {
        ...
        _ => panic!(
            "Method input params should be `(&self/&mut self, ctx: ServiceContext)` or `(&self/&mut self, ctx: ServiceContext, payload: PayloadType)`"
        ),
    }
}
