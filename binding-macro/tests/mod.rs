...
use protocol::traits::{
    ExecutorParams, Service, ServiceResponse, ServiceSDK, StoreArray, StoreBool,
    StoreMap, StoreString, StoreUint64,
};
use protocol::types::{
    Address, Block, Hash, Receipt, ServiceContext, ServiceContextParams,
    SignedTransaction,
};
...
fn test_service() {
    ...
    let read_res =
        panic::catch_unwind(AssertUnwindSafe(|| test_service.read_(context.clone())));
    assert_eq!(read_res.unwrap().is_error(), true);
    let write_res =
        panic::catch_unwind(AssertUnwindSafe(|| test_service.write_(context)));
...
impl ServiceSDK for MockServiceSDK {
    ...
    fn alloc_or_recover_map<
        Key: 'static + FixedCodec + PartialEq,
