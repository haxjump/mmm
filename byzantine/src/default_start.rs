...
use bytes::Bytes;
use common_crypto::{
    BlsCommonReference, BlsPrivateKey, BlsPublicKey, PublicKey, Secp256k1PrivateKey,
...
use crate::message::{
    ChokeMessageHandler, NewTxsHandler, ProposalMessageHandler, PullTxsHandler,
    QCMessageHandler, RemoteHeightMessageHandler, VoteMessageHandler,
};
...
pub async fn start(
    ...
    for validator_extend in metadata.verifier_list.iter() {
        ...
        let pub_key = BlsPublicKey::try_from(hex_pubkey.as_ref())
            .expect("try into BlsPublicKey failed");
    ...
    let bls_priv_key = BlsPrivateKey::try_from(priv_key.as_ref())
        .expect("try into BlsPrivateKey failed");

    let hex_common_ref = hex::decode(metadata.common_ref.as_string_trim0x())
        .expect("decode common ref failed");
    ...
    network_service.register_endpoint_handler(
        END_GOSSIP_NEW_TXS,
        NewTxsHandler::new(network_tx.clone()),
    )?;

    // register pull txs from other node
    network_service.register_endpoint_handler(
        RPC_PULL_TXS,
        PullTxsHandler::new(network_tx.clone()),
    )?;
