...
use crate::utils::{
    ...
    gen_invalid_content_struct_proposal, gen_invalid_from, gen_invalid_hash,
    gen_invalid_lock, gen_invalid_proof, gen_invalid_request, gen_invalid_sig,
    gen_invalid_validators, gen_positive_range, gen_random_bytes, gen_range,
    gen_signed_proposal_from_header, gen_signed_tx, gen_valid_block,
    gen_valid_block_header, gen_valid_choke, gen_valid_hash, gen_valid_proposal,
    gen_valid_qc, gen_valid_raw_tx, gen_valid_signed_choke, gen_valid_signed_proposal,
    gen_valid_signed_tx, gen_valid_signed_vote, gen_valid_vote,
};
...
pub fn gen_valid_new_proposal(
    ...
    let order_tx_hashes: Vec<Hash> =
        (0..gen_range(0, 1000)).map(|_| gen_valid_hash()).collect();
    let propose_tx_hashes: Vec<Hash> =
        (0..gen_range(0, 1000)).map(|_| gen_valid_hash()).collect();
...
pub fn gen_invalid_validators_new_proposal(
    ...
    let mut header =
        gen_valid_block_header(state, metadata, my_address, validators, vec![]);
