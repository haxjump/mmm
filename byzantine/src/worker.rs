...
use overlord::types::{
    AggregatedVote, Choke, PoLC, Proposal, SignedChoke, SignedProposal, SignedVote,
    Vote, VoteType,
};
use rlp::Encodable;
...
use crate::behaviors::{
    Behavior, MessageType, NewChoke, NewProposal, NewQC, NewTx, NewVote, PullTxs,
    Request,
};
use crate::invalid_types::{
    ...
    gen_invalid_exec_height_new_proposal, gen_invalid_from_new_vote,
    gen_invalid_hash_pull_txs, gen_invalid_hash_signed_tx, gen_invalid_height_new_choke,
    gen_invalid_height_new_proposal, gen_invalid_height_new_qc,
    gen_invalid_height_new_vote, gen_invalid_height_pull_txs, gen_invalid_leader_new_qc,
    gen_invalid_lock_new_proposal, gen_invalid_nonce_dup_signed_tx,
    ...
    gen_invalid_round_new_choke, gen_invalid_round_new_proposal,
    gen_invalid_round_new_qc, gen_invalid_round_new_vote, gen_invalid_sender_signed_tx,
    ...
    gen_invalid_signed_tx_hash_new_proposal, gen_invalid_state_root_new_proposal,
    gen_invalid_struct_new_choke, gen_invalid_struct_new_proposal,
    gen_invalid_struct_new_qc, gen_invalid_struct_new_vote,
    ...
    gen_invalid_version_new_proposal, gen_invalid_voter_new_vote,
    gen_not_exists_txs_pull_txs, gen_valid_new_proposal, gen_valid_tx, InvalidStruct,
};
use crate::utils::{
    gen_positive_range, gen_random_bytes, gen_valid_signed_choke, gen_valid_signed_vote,
    time_now,
};
    ...
    ($self_: ident, $ctx: ident, $behavior: ident, $func: ident) => {{
        ...
        tokio::spawn(async move {
            let messages: Vec<Vec<u8>> = (0..behavior.msg_num)
                .map(|_| {
                    $func(&state, &metadata, &crypto, &address, &pub_key, &validators)
                })
                .collect();
            for msg in messages {
                send(
    ...
    N: Rpc + PeerTrust + Gossip + 'static,
{
    ...
    pub async fn run(mut self) {
        ...
        loop {
            let (ctx, behaviors) =
                self.from_timeout.next().await.expect("Channel is down!");
    ...
    pub async fn process(&mut self, ctx: Context, behavior: &Behavior) {
        match &behavior.msg_type {
            MessageType::NewTxs(new_tx) => match new_tx {
                NewTx::InvalidStruct => {
                    self.send_invalid_struct_of_new_tx(ctx, behavior).await
                }
                NewTx::InvalidHash => {
                    send_new_tx!(self, ctx, behavior, gen_invalid_hash_signed_tx)
                }
                NewTx::InvalidSig => {
                    send_new_tx!(self, ctx, behavior, gen_invalid_sig_signed_tx)
                }
                ...
                NewTx::InvalidNonceOfRandLen => {
                    send_new_tx!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_nonce_of_rand_len_signed_tx
                    )
                }
            ...
            MessageType::RecvProposal(pull_txs) => match pull_txs {
                ...
                PullTxs::InvalidStruct => {
                    self.send_invalid_struct_of_pull_txs(ctx, behavior).await
                }
            },
            MessageType::SendProposal(new_proposal) => match new_proposal {
                ...
                NewProposal::InvalidStruct => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_struct_new_proposal
                    )
                }
                ...
                NewProposal::InvalidPrevHash => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_prev_hash_new_proposal
                    )
                }
                NewProposal::InvalidHeight => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_height_new_proposal
                    )
                }
                NewProposal::InvalidExecHeight => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_exec_height_new_proposal
                    )
                }
                NewProposal::InvalidTimestamp => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_timestamp_new_proposal
                    )
                }
                NewProposal::InvalidOrderRoot => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_order_root_new_proposal
                    )
                }
                ...
                NewProposal::InvalidConfirmRoot => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_confirm_root_new_proposal
                    )
                }
                NewProposal::InvalidStateRoot => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_state_root_new_proposal
                    )
                }
                NewProposal::InvalidReceiptRoot => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_receipt_root_new_proposal
                    )
                }
                NewProposal::InvalidCyclesUsed => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_cycle_used_new_proposal
                    )
                }
                ...
                NewProposal::InvalidVersion => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_version_new_proposal
                    )
                }
                NewProposal::InvalidValidators => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_validators_new_proposal
                    )
                }
                NewProposal::InvalidTxHash => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_tx_hash_new_proposal
                    )
                }
                ...
                NewProposal::InvalidProposalHeight => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_prop_height_new_proposal
                    )
                }
                NewProposal::InvalidRound => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_round_new_proposal
                    )
                }
                NewProposal::InvalidContentStruct => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_content_struct_new_proposal
                    )
                }
                NewProposal::InvalidBlockHash => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_block_hash_new_proposal
                    )
                }
                ...
                NewProposal::InvalidProposalProposer => {
                    send_new_proposal!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_prop_proposer_new_proposal
                    )
                }
            },
            ...
            MessageType::SendQC(new_qc) => match new_qc {
                ...
                NewQC::InvalidRound => {
                    send_new_qc!(self, ctx, behavior, gen_invalid_round_new_qc)
                }
                NewQC::InvalidBlockHash => {
                    send_new_qc!(self, ctx, behavior, gen_invalid_block_hash_new_qc)
                }
                NewQC::InvalidSig => {
                    send_new_qc!(self, ctx, behavior, gen_invalid_sig_new_qc)
                }
                NewQC::InvalidLeader => {
            ...
            MessageType::PullTxs(new_tx) => match new_tx {
                NewTx::InvalidStruct => {
                    self.send_invalid_struct_of_push_txs(ctx, behavior).await
                }
                NewTx::InvalidHash => {
                    send_push_txs!(self, ctx, behavior, gen_invalid_hash_signed_tx)
                }
                NewTx::InvalidSig => {
                    send_push_txs!(self, ctx, behavior, gen_invalid_sig_signed_tx)
                }
                NewTx::InvalidChainID => {
                ...
                NewTx::InvalidCyclesPrice => {
                    send_push_txs!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_cycles_price_signed_tx
                    )
                }
                NewTx::InvalidCyclesLimit => {
                    send_push_txs!(
                        ...
                        ctx,
                        behavior,
                        gen_invalid_cycles_limit_signed_tx
                    )
                }
                NewTx::InvalidNonceOfRandLen => {
                    send_push_txs!(
    ...
    pub async fn send_invalid_struct_of_pull_txs(
        &mut self,
        ctx: Context,
        behavior: &Behavior,
    ) {
    ...
    pub async fn send_invalid_struct_of_new_tx(
        &self,
        ctx: Context,
        behavior: &Behavior,
    ) {
    ...
    pub async fn send_invalid_struct_of_push_txs(
        &self,
        ctx: Context,
        behavior: &Behavior,
    ) {
    ...
    pub async fn send_invalid_nonce_dup_of_new_tx(
        &self,
        ctx: Context,
        behavior: &Behavior,
    ) {
        ...
        tokio::spawn(async move {
            let batch_stxs: Vec<SignedTransaction> = (0..behavior.msg_num)
                .map(|_| {
                    gen_invalid_nonce_dup_signed_tx(
                        &TEST_PRI_KEY,
                        height,
                        &metadata,
                        nonce.clone(),
            ...
            match req {
                ...
                Request::RecvQC(qc) => {
                    let qc: AggregatedVote =
                        rlp::decode(&qc.0).expect("decode qc failed");
                    if !qc.is_prevote_qc() && qc.height >= self.state.height {
                        if !qc.block_hash.is_empty() {
                            ...
                            self.state.prev_hash =
                                Hash::from_bytes(qc.block_hash.clone()).unwrap();
                            self.state.proof = Proof {
                                height: qc.height,
                                round: qc.round,
                                block_hash: Hash::from_bytes(qc.block_hash.clone())
                                    .unwrap(),
                ...
                Request::RecvVote(vote) => {
                    let vote: SignedVote =
                        rlp::decode(&vote.0).expect("decode vote failed");
                ...
                Request::RecvChoke(choke) => {
                    let choke: SignedChoke =
                        rlp::decode(&choke.0).expect("decode choke failed");
