...
use bytes::Bytes;
use overlord::types::{
    AggregatedChoke, AggregatedSignature, AggregatedVote, Choke, PoLC, Proposal,
    SignedChoke, SignedProposal, SignedVote, UpdateFrom, Vote, VoteType,
};
use overlord::Crypto;
...
use protocol::types::{
    Address, Block, BlockHeader, Hash, Metadata, Pill, Proof, RawTransaction,
...
pub fn gen_bool(p: f64) -> bool {
    let mut rng = rand::thread_rng();
    if p >= 1.0 { true } else { rng.gen_bool(p) }
}
...
pub fn gen_address_bytes(pri_key: &Secp256k1PrivateKey) -> Address {
    ...
    Address::from_pubkey_bytes(pubkey.to_uncompressed_bytes())
        .expect("get address failed")
}
...
pub fn gen_signed_tx(
    ...
    let fixed_bytes = fixed_bytes
        .unwrap_or_else(|| raw.encode_fixed().expect("get bytes from raw_tx failed!"));
