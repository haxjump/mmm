use crate::metrics::{
    auto_flush_from, exponential_buckets, make_auto_flush_static_metric,
...
lazy_static! {
    pub static ref CONSENSUS_RESULT_COUNTER_VEC: IntCounterVec =
...
lazy_static! {
    ...
    pub static ref ENGINE_EXECUTING_BLOCK_GAUGE: IntGauge =
        register_int_gauge!("muta_executing_block_count", "The executing blocks")
            .unwrap();
    ...
    .unwrap();
    pub static ref ENGINE_ORDER_TX_GAUGE: IntGauge = register_int_gauge!(
        "muta_proposal_order_tx_len",
        "The ordered transactions len"
    )
    .unwrap();
    pub static ref ENGINE_SYNC_TX_GAUGE: IntGauge =
        register_int_gauge!("muta_proposal_sync_tx_len", "The sync transactions len")
            .unwrap();
