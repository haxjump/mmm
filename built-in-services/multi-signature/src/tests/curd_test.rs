...
use crate::types::{
    ...
    MultiSigPermission, RemoveAccountPayload, SetAccountWeightPayload,
    SetThresholdPayload, UpdateAccountPayload,
};
...
fn test_generate_multi_signature() {
    ...
    let accounts = gen_keypairs(17)
        ...
        .collect::<Vec<_>>();
    let multi_sig_address = service.generate_account(
        context.clone(),
    ...
    let permission = service.get_account_from_address(
        context,
    ...
    assert_eq!(
        permission.succeed_data.permission,
        MultiSigPermission {
            owner,
...
fn test_add_account() {
    ...
    let permission = service.get_account_from_address(
        context,
        GetMultiSigAccountPayload { multi_sig_address },
    );
    assert_eq!(
        permission.succeed_data.permission,
        MultiSigPermission {
...
fn test_update_account() {
    ...
    let res = service.update_account(
        context.clone(),
...
fn test_set_weight() {
    ...
    let multi_sig_address = service
        ...
        .succeed_data
        .address;
    let to_be_changed_address =
        Address::from_pubkey_bytes(keypairs[0].1.clone()).unwrap();

    // test set weight success
    let res = service.set_account_weight(
        context.clone(),
...
fn test_remove_account() {
    ...
    let multi_sig_address = service
        ...
        .succeed_data
        .address;
    let to_be_removed_address =
        Address::from_pubkey_bytes(keypairs[3].1.clone()).unwrap();

    let res = service.remove_account(
        context.clone(),
    ...
    let to_be_removed_address =
        Address::from_pubkey_bytes(keypairs[2].1.clone()).unwrap();
    let res = service.remove_account(
        context.clone(),
        RemoveAccountPayload {
            multi_sig_address: multi_sig_address.clone(),
