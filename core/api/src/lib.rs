...
use crate::schema::{
    to_signed_transaction, to_transaction, Address, Block, Bytes, Hash,
    InputRawTransaction, InputTransactionEncryption, Receipt, ServiceResponse,
...
mod profile {
    ...
    impl FromRequest for ProfileConfig {
        ...
        fn from_request(req: &HttpRequest, _: &mut dev::Payload) -> Self::Future {
            ...
            let duration: Duration =
                match query_pairs.get("duration").map(|val| val.parse()) {
            ...
            let frequency: i32 =
                match query_pairs.get("frequency").map(|val| val.parse()) {
                    Some(Ok(val)) => val,
                    Some(Err(e)) => return future::err(ErrorBadRequest(e)),
            ...
            let report: ProfileReport =
                match query_pairs.get("report").map(|val| val.parse()) {
                    Some(Ok(val)) => val,
                    Some(Err(e)) => return future::err(ErrorBadRequest(e)),
    ...
    pub async fn dump_profile(
        maybe_config: actix_web::Result<ProfileConfig>,
    ) -> HttpResponse {
...
pub async fn start_graphql<Adapter: APIAdapter + 'static>(
    cfg: GraphQLConfig,
    adapter: Adapter,
) {
    ...
    let server = HttpServer::new(move || {
        ...
        if enable_dump_profile {
            app.service(
                web::resource("/dump_profile")
                    .route(web::get().to(profile::dump_profile)),
            )
        } else {
            app
        }
