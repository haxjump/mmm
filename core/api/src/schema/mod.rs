...
pub use transaction::{
    to_signed_transaction, to_transaction, InputRawTransaction,
    InputTransactionEncryption, SignedTransaction,
};
...
impl Uint64 {
    ...
    pub fn try_into_u64(self) -> ProtocolResult<u64> {
        let n =
            u64::from_str_radix(&self.as_hex()?, 16).map_err(SchemaError::IntoU64)?;
        Ok(n)
    }
}
