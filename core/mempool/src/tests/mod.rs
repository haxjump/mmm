...
use protocol::types::{
    Address, Hash, RawTransaction, SignedTransaction, TransactionRequest,
};
...
fn check_sig(tx: &SignedTransaction) -> ProtocolResult<()> {
    if Secp256k1::verify_signature(&tx.tx_hash.as_bytes(), &tx.signature, &tx.pubkey)
        .is_err()
    {
...
fn check_order_consistant(
    mixed_tx_hashes: &MixedTxHashes,
