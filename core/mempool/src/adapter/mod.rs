...
use futures::{
    channel::mpsc::{
        channel, unbounded, Receiver, Sender, TrySendError, UnboundedReceiver,
        UnboundedSender,
    },
    lock::Mutex,
    select,
...
use protocol::{
    ...
    traits::{
        Context, ExecutorFactory, ExecutorParams, Gossip, MemPoolAdapter, PeerTrust,
        Priority, Rpc, ServiceMapping, ServiceResponse, Storage, TrustFeedback,
    },
...
#[async_trait]
impl<EF, C, N, S, DB, Mapping> MemPoolAdapter
    for DefaultMemPoolAdapter<EF, C, N, S, DB, Mapping>
where
    EF: ExecutorFactory<DB, S, Mapping>,
    ...
    async fn check_authorization(
        ...
        let blocking_res: ProtocolResult<ServiceResponse<String>> =
            tokio::task::spawn_blocking(move || {
                ...
                let stx_ptr_json =
                    format!("{{ \"ptr\": {} }}", Box::into_raw(tx) as usize);
                let check_resp = executor.read(
                    &params,
                    &caller,
                    1,
                    &TransactionRequest {
    ...
    async fn get_transactions_from_storage(
        ...
        if let Some(height) = block_height {
            ...
            let futs = tx_hashes
                .iter()
                .map(|tx_hash| {
                    self.storage.get_transaction_by_hash(ctx.clone(), tx_hash)
                })
                .collect::<Vec<_>>();
