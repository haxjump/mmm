...
use protocol::traits::{
    ConsensusAdapter, Context, MessageTarget, NodeInfo, TrustFeedback,
};
use protocol::types::{
    Address, Block, BlockHeader, Hash, MerkleRoot, Metadata, Pill, Proof,
...
use crate::util::{
    check_list_roots, digest_signed_transactions, time_now, OverlordCrypto,
};
...
#[async_trait]
impl<Adapter: ConsensusAdapter + 'static> Engine<FixedPill>
    for ConsensusEngine<Adapter>
{
    ...
    async fn get_block(
        ...
        {
            error!(
                "[consensus] get_block for {}, error, current_consensus_status.current_height {} != current_consensus_status.current_proof.height, proof :{:?}",
    ...
    async fn check_block(
        ...
        if block.inner.block.header.height != block.inner.block.header.proof.height + 1 {
            error!(
                "[consensus-engine]: check_block for overlord receives a proposal, error, block height {}, block {:?}",
                block.inner.block.header.height, block.inner.block
            );
        }
    ...
    async fn commit(
        ...
        if current_consensus_status.exec_height == current_height {
            let status = Status {
                ...
                authority_list: covert_to_overlord_authority(
                    &current_consensus_status.validators,
                ),
            };
            return Ok(status);
        }
        ...
        common_apm::metrics::consensus::ENGINE_ROUND_GAUGE
            .set(commit.proof.round as i64);
    ...
    fn report_error(&self, ctx: Context, err: OverlordError) {
        match err {
            OverlordError::CryptoErr(_) | OverlordError::AggregatedSignatureErr(_) => {
                self.adapter
...
impl<Adapter: ConsensusAdapter + 'static> ConsensusEngine<Adapter> {
    ...
    fn check_block_roots(
            ...
            && !status.list_state_root.contains(&block.state_root)
        {
            warn!(
                ...
                status.latest_committed_state_root,
                status.list_state_root,
                block.state_root
            );
    ...
    pub async fn update_status(
        ...
        if block.header.height != proof.height {
            info!(
                "[consensus] update_status for handle_commit, error, before update, block height {}, proof height:{}, proof : {:?}",
                block.header.height,
                proof.height,
                proof.clone()
            );
        ...
        self.status_agent.update_by_committed(
            metadata.clone(),
            block,
            block_hash,
            proof,
        ...
        {
            error!(
                "[consensus] update_status for handle_commit, error, current_height {} != current_proof.height {}, proof :{:?}",
    ...
    fn metric_commit(&self, current_height: u64, txs_len: usize) {
        common_apm::metrics::consensus::ENGINE_HEIGHT_GAUGE
            .set((current_height + 1) as i64);
        common_apm::metrics::consensus::ENGINE_COMMITED_TX_COUNTER
            .inc_by(txs_len as i64);
...
pub fn generate_new_crypto_map(
    metadata: Metadata,
) -> ProtocolResult<HashMap<Bytes, BlsPublicKey>> {
        ...
        let hex_pubkey =
            hex::decode(validator.bls_pub_key.as_string_trim0x()).map_err(|err| {
                ConsensusError::Other(format!(
                    "hex decode metadata bls pubkey error {:?}",
                    err
                ))
            })?;
...
fn gauge_txs_len(pill: &Pill) {
    ...
    common_apm::metrics::consensus::ENGINE_SYNC_TX_GAUGE
        .set(pill.propose_hashes.len() as i64);
}
