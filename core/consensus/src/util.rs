...
use common_crypto::{
    BlsCommonReference, BlsPrivateKey, BlsPublicKey, BlsSignature, BlsSignatureVerify,
    HashValue, PrivateKey, Signature,
};
...
impl Crypto for OverlordCrypto {
    ...
    fn aggregate_signatures(
        ...
        for (sig, addr) in signatures.iter().zip(voters.iter()) {
            let signature = BlsSignature::try_from(sig.as_ref()).map_err(|e| {
                ProtocolError::from(ConsensusError::CryptoErr(Box::new(e)))
            })?;
...
pub fn digest_signed_transactions(
...
mod tests {
    ...
    fn test_convert_from_hex() {
        ...
        assert!(
            convert_hex_to_bls_pubkeys(Hex::from_string(String::from(hex_str)).unwrap())
                .is_ok()
        );
    }
}
