...
pub use crate::fixed_types::{
    FixedBlock, FixedHeight, FixedProof, FixedSignedTxs, PullTxsRequest,
};
...
impl<Sy: Synchronization + 'static> MessageHandler for RemoteHeightMessageHandler<Sy> {
    ...
    #[muta_apm::derive::tracing_span(
        name = "handle_remote_height",
        kind = "consensus.message"
    )]
    async fn process(
        &self,
        ctx: Context,
        remote_height: Self::Message,
    ) -> TrustFeedback {
        if let Err(e) = self
            .synchronization
...
#[async_trait]
impl<R: Rpc + 'static, S: Storage + 'static> MessageHandler
    for PullBlockRpcHandler<R, S>
{
    ...
    async fn process(&self, ctx: Context, msg: FixedHeight) -> TrustFeedback {
        ...
        self.rpc
            ...
            .unwrap_or_else(move |e: ProtocolError| {
                warn!("[core_consensus] push block {}", e)
            })
            .await;
    ...
    for PullProofRpcHandler<R, S>
{
    ...
    async fn process(&self, ctx: Context, height: FixedHeight) -> TrustFeedback {
        ...
        self.rpc
            ...
            .unwrap_or_else(move |e: ProtocolError| {
                warn!("[core_consensus] push proof {}", e)
            })
            .await;
...
impl<R: Rpc + 'static, S: Storage + 'static> MessageHandler for PullTxsRpcHandler<R, S> {
    ...
    async fn process(&self, ctx: Context, msg: PullTxsRequest) -> TrustFeedback {
        ...
        self.rpc
            ...
            .unwrap_or_else(move |e: ProtocolError| {
                warn!("[core_consensus] push txs {}", e)
            })
            .await;
