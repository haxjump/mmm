...
use overlord::types::{
    AggregatedSignature, AggregatedVote, Node, SignedVote, Vote, VoteType,
};
...
use protocol::traits::{
    CommonConsensusAdapter, Synchronization, SynchronizationAdapter,
};
use protocol::traits::{
    Context, ExecutorParams, ExecutorResp, ServiceResponse, TrustFeedback,
};
use protocol::types::{
    Address, Block, BlockHeader, Bytes, Hash, Hex, MerkleRoot, Metadata, Proof,
    RawTransaction, Receipt, ReceiptResponse, SignedTransaction, TransactionRequest,
...
use crate::util::{
    convert_hex_to_bls_pubkeys, digest_signed_transactions, OverlordCrypto,
};
...
fn sync_gap_test() {
    for gap in [1, 2, 3, 4].iter() {
        ...
        let block = block_on(
            adapter.get_block_by_height(Context::new(), status.latest_committed_height),
        )
        .unwrap();
...
impl CommonConsensusAdapter for MockCommonConsensusAdapter {
    ...
    async fn verify_block_header(
        ...
        if block.header.height > 1u64
            && block.header.prev_hash != block.header.proof.block_hash
        {
            log::error!(
    ...
    async fn verify_proof(
        ...
        if blockhash != proof.block_hash {
            ...
            return Err(
                ConsensusError::VerifyProof(block_header.height, HashMismatch).into(),
            );
        }
        ...
        let signed_voters =
            extract_voters(&mut authority_list, &proof.bitmap).map_err(|_| {
    ...
    fn verify_proof_signature(
        ...
        self.crypto
            .inner_verify_aggregated_signature(
                vote_hash,
                pub_keys,
            ...
            .map_err(|e| {
                ...
                ConsensusError::VerifyProof(block_height, BlockProofField::Signature)
                    .into()
            })
    }
    ...
    fn verify_proof_weight(
        ...
        let total_validator_weight: u64 =
            weight_map.iter().map(|pair| u64::from(*pair.1)).sum();
...
fn mock_chained_rich_block(
    len: u64,
    gap: u64,
    key_tool: &KeyTool,
) -> (Vec<RichBlock>, Vec<Proof>) {
    ...
    loop {
        ...
        last_proof =
            mock_proof(current_block_hash.clone(), current_height, 0, &key_tool);
...
fn get_mock_public_keys_and_common_ref()
-> (HashMap<Bytes, BlsPublicKey>, BlsCommonReference) {
