...
use protocol::types::{
    Address, Block, BlockHeader, Hash, Hex, MerkleRoot, Proof, Validator,
};
use protocol::Bytes;
...
fn mock_current_status(exec_lag: u64) -> CurrentConsensusStatus {
    ...
    CurrentConsensusStatus {
        ...
        latest_committed_state_root: state_roots
            .last()
            .cloned()
            .unwrap_or_else(mock_hash),
...
fn mock_pub_key() -> Hex {
    Hex::from_string(
        "0x026c184a9016f6f71a234c86b141621f38b68c78602ab06768db4d83682c616004"
            .to_owned(),
    )
    .unwrap()
}
