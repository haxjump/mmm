...
impl<Adapter: ConsensusAdapter + 'static> Consensus for OverlordConsensus<Adapter> {
    ...
    async fn set_vote(&self, ctx: Context, vote: Vec<u8>) -> ProtocolResult<()> {
        let ctx = match muta_apm::MUTA_TRACER.span(
            "consensus.set_vote",
            vec![muta_apm::rustracing::tag::Tag::new("kind", "consensus")],
        ) {
            Some(mut span) => {
                span.log(|log| {
        ...
        let signed_vote: SignedVote = rlp::decode(&vote)
...
impl<Adapter: ConsensusAdapter + 'static> OverlordConsensus<Adapter> {
    pub fn new(
        ...
        let overlord =
            Overlord::new(node_info.self_pub_key, Arc::clone(&engine), crypto, engine);
