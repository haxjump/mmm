...
impl NetworkConfig {
    pub fn new() -> Self {
        ...
        NetworkConfig {
            ...
            discovery_sync_interval: Duration::from_secs(
                DEFAULT_DISCOVERY_SYNC_INTERVAL,
            ),
    ...
    pub fn allowlist<S: AsRef<[String]>>(
        mut self,
        peer_id_strs: S,
    ) -> ProtocolResult<Self> {
        let peer_ids = {
...
impl From<&NetworkConfig> for PeerManagerConfig {
    fn from(config: &NetworkConfig) -> PeerManagerConfig {
        let peer_trust_config = TrustMetricConfig::new(
            config.peer_trust_interval,
            config.peer_trust_max_history,
        );
