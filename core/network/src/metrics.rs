    ...
    fn report_pending_data(&self) {
        ...
        let total_size: usize = sids
            ...
            .map(|sid| {
                ...
                if let Some(ConnectedAddr { host, .. }) =
                    self.sessions.connected_addr(*sid)
                {
                    let guage =
                        common_apm::metrics::network::NETWORK_IP_PENDING_DATA_SIZE_VEC
        ...
        common_apm::metrics::network::NETWORK_TOTAL_PENDING_DATA_SIZE
