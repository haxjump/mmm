...
impl Rpc for NetworkRpc {
        ...
        R: MessageCodec,
    {
        ...
        let ret = match future::select(done_rx, timeout).await {
            Either::Left((ret, _timeout)) => ret.map_err(|_| {
                NetworkError::from(ErrorKind::RpcDropped(connected_addr))
            })?,
            Either::Right((_unresolved, _timeout)) => {
                ...
                return Err(
                    NetworkError::from(ErrorKind::RpcTimeout(connected_addr)).into()
                );
            }
        };
        ...
        match ret {
            ...
            RpcResponse::Error(e) => {
                Err(NetworkError::RemoteResponse(Box::new(e)).into())
            }
        }
    }
