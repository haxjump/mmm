...
impl StateContext {
    ...
    pub fn send_message(&self, msg: Bytes) {
        if let Err(err) = self.service_control.quick_send_message_to(
            self.remote_peer.sid,
            self.proto_id,
            msg,
        ) {
            log::warn!(
...
impl IdentifyProtocol {
    ...
    pub fn wait_failed(peer_id: &PeerId, error: String) {
        if let Some(identification) =
            { PEER_IDENTIFICATION_BACKLOG.write().remove(peer_id) }
        {
    ...
    pub fn on_received(
        &mut self,
        protocol_context: &IdentifyProtocolContext,
        data: Bytes,
    ) {
        {
            ...
            State::ServerNegotiate {
                ...
                ServerProcedure::WaitIdentity => {
                    let identity = match Identity::decode(data) {
                        ...
                        Err(_) => {
                            log::warn!(
                                "received invalid identity from {:?}",
                                context.remote_peer
                            );
