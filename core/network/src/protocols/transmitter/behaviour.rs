...
impl<'a> SendingContext<'a> {
    fn send(&self, msg: TransmitterMessage) -> Result<(), NetworkError> {
        ...
        match msg.recipient {
            Recipient::Session(target) => {
                self.send_to_sessions(target, data, priority, msg.ctx)
            }
            Recipient::PeerId(peer_ids) => {
                self.send_to_peers(peer_ids, data, priority, msg.ctx)
            }
        }
    }
    ...
    fn send_to_peers(
        ...
        let send_ret = self.send_to_sessions(
            TargetSession::Multi(connected),
            data,
            priority,
            msg_ctx,
        );
