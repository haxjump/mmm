...
use tokio_util::codec::{
    length_delimited::LengthDelimitedCodec, Decoder, Encoder, Framed,
};
        ...
        match message {
            DiscoveryMessage {
                ...
                if self.received_get_nodes {
                    ...
                    let announce = false;
                    let items =
                        items.into_iter().map(|addr| vec![addr]).collect::<Vec<_>>();
            ...
            DiscoveryMessage {
                ...
                for item in &nodes.items {
                    if item.addrs.len() > MAX_ADDRS {
                        let misbehavior =
                            Misbehavior::TooManyAddresses(item.addrs.len());
                        if addr_mgr
