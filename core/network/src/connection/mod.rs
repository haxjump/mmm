        ...
        match event {
            ConnectionEvent::Connect { addrs, .. } => {
                ...
                for addr in addrs.into_iter() {
                    if let Err(()) =
                        try_do!(control.dial(addr.clone(), target_protocol.clone()))
                    {
...
impl<P: NetworkProtocol + Unpin> Future for ConnectionService<P> {
    ...
    fn poll(mut self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let mut pending_events =
            std::mem::replace(&mut serv_mut.pending_events, VecDeque::new());
        ...
        while serv_mut.pending_events.is_empty() {
            ...
            let event =
                crate::service_ready!("connection service", event_rx.poll_next(ctx));
