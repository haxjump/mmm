...
use crate::event::{
    ConnectionErrorKind, ConnectionEvent, ConnectionType, MisbehaviorKind,
    PeerManagerEvent, SessionErrorKind,
};
...
impl PeerManager {
    ...
    fn new_session_pre_check(
        ...
        let remote_multiaddr =
            PeerMultiaddr::new(ctx.address.to_owned(), &remote_peer_id);
        ...
        let remote_peer =
            opt_peer.unwrap_or_else(|| ArcPeer::new(remote_peer_id.clone()));
                ...
                for session in self.inner.share_sessions() {
                    let session_trust_score = match session.peer.trust_metric() {
                        ...
                        None => {
                            // Impossible
                            error!(
                                "session peer {:?} trust metric not found",
                                session.peer.id
                            );
                            return false;
                        }
    ...
    fn new_session(&mut self, pubkey: PublicKey, ctx: Arc<SessionContext>) {
        ...
        match remote_peer.trust_metric() {
            ...
            None => {
                let trust_metric =
                    TrustMetric::new(Arc::clone(&self.config.peer_trust_config));
                trust_metric.start();
    ...
    fn session_closed(&mut self, pid: PeerId, sid: SessionId) {
        ...
        let remote_peer = {
            match opt_session
                .map_or_else(|| self.inner.peer(&pid), |s| Some(s.peer.to_owned()))
            {
                Some(peer) => peer,
                None => {
        ...
        remote_peer.mark_disconnected();
        if remote_peer.tags.contains(&PeerTag::Consensus)
            && opt_unidentified_session.is_none()
        {
    ...
    fn connect_failed(&mut self, addr: Multiaddr, error_kind: ConnectionErrorKind) {
        use ConnectionErrorKind::{
            DNSResolver, Io, MultiaddrNotSuppored, PeerIdNotMatch, ProtocolHandle,
            SecioHandshake, TimeOut,
        };
            ...
            Bad(_) | Worse(_) => {
                    ...
                    && !peer.tags.contains(&PeerTag::Consensus)
                {
                    ...
                    if let Err(e) = peer.tags.insert_ban(Duration::from_secs(soft_ban)) {
                        warn!("ban peer {}", e);
                        debug!(
                            "impossible, we already make sure peer isn't in allowlist"
                        );
                    }
    ...
    fn connect_peers(&mut self, peers: Vec<ArcPeer>) {
        ...
        let connectable_peers: Vec<_> =
            peers.into_iter().filter_map(connectable).collect();
    ...
    fn process_event(&mut self, event: PeerManagerEvent) {
        match event {
            ...
            PeerManagerEvent::ConnectFailed { addr, kind } => {
                self.connect_failed(addr, kind)
            }
            PeerManagerEvent::UnidentifiedSession { pubkey, ctx, .. } => {
                self.new_unidentified_session(pubkey, ctx)
            }
            PeerManagerEvent::NewSession { pubkey, ctx, .. } => {
                self.new_session(pubkey, ctx)
            }
            ...
            PeerManagerEvent::SessionClosed { sid, pid } => {
                self.session_closed(pid, sid)
            }
            PeerManagerEvent::SessionFailed { sid, kind } => {
                self.session_failed(sid, kind)
            }
            ...
            PeerManagerEvent::DiscoverMultiAddrs { addrs } => {
                self.dicover_multi_multiaddrs(addrs)
            }
            PeerManagerEvent::IdentifiedAddrs { pid, addrs } => {
                self.identified_addrs(&pid, addrs)
            }
...
impl Future for PeerManager {
    ...
    fn poll(mut self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let unidentified_sessions =
            self.unidentified_backlog.drain().collect::<Vec<_>>();
        ...
        let timeout_reason =
            format!("exceed {} seconds", MAX_CONNECTING_TIMEOUT.as_secs());
        ...
        let max_connection_attempts =
            self.config.outbound_conn_limit + MAX_CONNECTING_MARGIN;
            ...
            && connection_attempts < max_connection_attempts
        {
            ...
            if connectable_peers.is_empty() {
                connectable_peers =
                    self.inner.connectable_peers(remain_count, just_enough);
            }
