...
use protocol::traits::{
    IntoIteratorByRef, StorageAdapter, StorageBatchModify, StorageCategory,
...
impl<'a, S: StorageSchema> Iterator for RocksIterator<'a, S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let kv_decode =
            |(k_bytes, v_bytes): (Box<[u8]>, Box<[u8]>)| -> ProtocolResult<_> {
...
impl<'c, S: StorageSchema, P: AsRef<[u8]>> IntoIteratorByRef<S>
    for RocksIntoIterator<'c, S, P>
{
...
impl StorageAdapter for RocksAdapter {
    ...
    async fn get<S: StorageSchema>(
        ...
        let opt_bytes = {
            db!(self.db, get_cf, column, key)?
                .map(|db_vec| Bytes::copy_from_slice(&db_vec))
        };
