...
use protocol::traits::{
    CommonStorage, Context, MaintenanceStorage, Storage, StorageAdapter,
...
macro_rules! get {
    ($self_: ident, $key: expr, $schema: ident) => {{ $self_.adapter.get::<$schema>($key).await }};
}
        ...
        let mut found = {
            if found.len() <= BATCH_VALUE_DECODE_NUMBER {
                found
                    .drain(..)
                    .map(|(k, v): (Hash, Bytes)| {
                        SignedTransaction::decode_sync(v).map(|v| (k, v))
                    })
...
impl<Adapter: StorageAdapter> CommonStorage for ImplStorage<Adapter> {
    ...
    async fn set_block(&self, _ctx: Context, block: Block) -> ProtocolResult<()> {
        self.adapter
            ...
            .await?;
        self.adapter
            .insert::<BlockHeaderSchema>(
                BlockKey::new(block.header.height),
                block.header.clone(),
            )
            .await?;
        Ok(())
