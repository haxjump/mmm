...
fn save_restore() {
    ...
    fs_extra::dir::remove(save.clone())
        .expect("save_restore, remove save_restore fails");
    ...
    let block =
        if let ("latest_block", Some(sub_cmd)) = maintenance_cli.matches.subcommand() {
            if let ("get", Some(_cmd)) = sub_cmd.subcommand() {
                let mut rt = tokio::runtime::Runtime::new().expect("new tokio runtime");
                let res =
                    rt.block_on(async move { maintenance_cli.latest_block_get().await });
...
fn latest_set() {
    ...
    if let ("latest_block", Some(sub_cmd)) = maintenance_cli.matches.subcommand() {
        if let ("set", Some(_cmd)) = sub_cmd.subcommand() {
            let mut rt = tokio::runtime::Runtime::new().expect("new tokio runtime");
            let res =
                rt.block_on(async move { maintenance_cli.latest_block_set(10).await });
            assert!(res.is_ok());
        } else {
            panic!()
        }
...
fn prepare() {
    ...
    for idx in 0..=23 {
        if let Err(e) = rt.block_on(storage.insert_block(
            Context::new(),
            Block {
                header: BlockHeader {
