    ...
    Mapping: 'static + ServiceMapping,
{
    ...
    pub fn new(
        ...
        let config: Config = common_config_parser::parse(&config_path.trim())
        ...
        match self.matches.subcommand() {
            ("run", Some(_sub_cmd)) => {
                ...
                if let Some(genesis) = self.genesis {
                    let muta =
                        run::Muta::new(self.config, genesis, self.service_mapping);
                    muta.run()
                } else {
    ...
    fn generate_maintenance_cli(
        self,
    ) -> MaintenanceCli<'a, Mapping, ImplStorage<RocksAdapter>> {
        let path_block = self.config.data_path_for_block();
        let rocks_adapter =
            match RocksAdapter::new(path_block, self.config.rocksdb.max_open_files) {
        ...
        match sub_cmd.subcommand() {
            ("set", Some(cmd)) => {
                ...
                match u64::from_str_radix(height, 10) {
                    Ok(height) => {
                        rt.block_on(async move { self.latest_block_set(height).await })
                    }
    ...
    pub fn backup(&self, sub_cmd: &ArgMatches) -> ProtocolResult<()> {
        match sub_cmd.subcommand() {
            ("save", Some(cmd)) => {
                ...
                self.backup_save(
                    PathBuf::from_str(to).map_err(|e| CliError::Path(e.to_string()))?,
                )
            }
